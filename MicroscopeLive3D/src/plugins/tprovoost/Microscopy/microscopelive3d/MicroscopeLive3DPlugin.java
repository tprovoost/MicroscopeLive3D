	...
	public void start() {
		...
		btn_start.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (ConfirmDialog.confirm("Are you sure ?", "<html>It will be impossible to manually change the Z position of<br/> "
						+ "the microscope while Live 3D Plugin is running.<br/>" + "<br/>Do you want to continue ?</html>")) {
	...
	class Live3DThread extends Thread {
		...
		public void run() {
			...
			ThreadUtil.invokeLater(new Runnable() {
				...
				@Override
				public void run() {
					new ToolTipFrame("<html><center><b>Tooltip</b><br/></center>To follow the acquisition progress of each stack,<br/>"
							+ "please refer to the \"Running Acquisitions\" panel<br/>in Micro-Manager For Icy GUI.</html>", getClass().getName() + "t1");
				}
			});
			while (!_stop) {
				while (_please_wait) {
				...
				try {
					...
					if (video == null) {
						...
						if (video.getSizeZ() != _slices || video.getWidth() != (int) mCore.getImageWidth() || video.getHeight() != (int) mCore.getImageHeight()) {
